...
def remove_emojis(text: str) -> str:
    ...
    if not text:
        return text
    # Remove emoji patterns (Unicode ranges for emojis)
...
def remove_tags(text: str, keep_collection_name: bool = True) -> str:
...
def clean_text(text: str) -> str:
    """Remove emojis and tags from text"""
    if not text:
        return text
...
def normalize_to_hierarchy(article_obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Removes emojis and tags from all text fields.
    """
    article = clean_text(str(article_obj.get("article", "")))
    section = clean_text(str(article_obj.get("section", "")))
    title = clean_text(str(article_obj.get("title", "")))
    ...
    if clauses and isinstance(clauses, list):
        ...
        for c in clauses:
            clean_clauses.append({
                "number": c.get("number"),
                "title": clean_text(str(c.get("title") or "")),
    ...
    # Flat -> wrap
    text = clean_text(str(article_obj.get("text", "")))
    if not text:
