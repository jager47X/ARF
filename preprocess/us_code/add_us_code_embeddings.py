# add_us_code_embeddings.py
"""
Add embeddings for United States Code (USC) documents in production MongoDB.
...
Skips documents/clauses that already have embeddings.
"""
import argparse
import logging
import os
...
# Parse arguments
def parse_args():
    parser = argparse.ArgumentParser(description="Add embeddings for US Code documents in production MongoDB")
    parser.add_argument(
        "--production",
        action="store_true",
...
logger = logging.getLogger("add_us_code_embeddings")
...
def compose_clause_text(clause: Dict[str, Any]) -> str:
    """Compose text for clause-level embedding."""
    parts = []
    if clause.get("title"):
...
def has_clause_embedding(clause: Dict[str, Any]) -> bool:
    """Check if clause has an embedding."""
    return clause.get("embedding") is not None and len(clause.get("embedding", [])) > 0
...
def process_document(embedder, doc: Dict[str, Any], dry_run: bool = False,
                     ...
                     clause_embedding_times: List[float] = None,
    ...
    if doc_embedding_times is None:
        doc_embedding_times = []
    if clause_embedding_times is None:
        clause_embedding_times = []
    ...
    # Check and generate clause-level embeddings
    clauses = doc.get("clauses", [])
    for clause_idx, clause in enumerate(clauses):
        if not has_clause_embedding(clause):
            clause_text = compose_clause_text(clause)
            if clause_text:
                try:
                    if not dry_run:
                        # Wait if needed to stay under rate limit
                        ...
                        if embedding_times is not None:
                            embedding_times.append(emb_time)
                        if clause_embedding_times is not None:
                            clause_embedding_times.append(emb_time)
                        if clause_emb is not None:
                            clause["embedding"] = clause_emb.tolist() if hasattr(clause_emb, 'tolist') else list(clause_emb)
                            updated = True
                            logger.debug(f"    Generated clause {clause_idx + 1} embedding ({emb_time:.3f}s)")
                    else:
                        logger.debug(f"    [DRY RUN] Would generate clause {clause_idx + 1} embedding")
                except Exception as e:
                    error_str = str(e).lower()
                    # Re-raise rate limit errors so they can be handled at a higher level
                    if "rate limit" in error_str or "rpm" in error_str:
                        raise
                    logger.warning(f"    Failed to generate clause {clause_idx + 1} embedding: {e}")
            else:
                logger.debug(f"    Clause {clause_idx + 1} has no text for embedding")
        else:
            logger.debug(f"    Clause {clause_idx + 1} already has embedding")

    return doc, updated

def main():
    """Main function to add embeddings for US Code documents."""
    client = None
    ...
    try:
        ...
        # Use custom Voyage API key for this script only
        # Set it BEFORE importing ai_service so it picks up the new value
        CUSTOM_VOYAGE_API_KEY = "pa-HBc-JUbxc4tjANFVZvAwmmTuUEpkH-zkFxKDxRAjwrV"
        if original_voyage_key is None:
            ...
            logger.info("Using custom Voyage API key for this script")
        ...
        for doc in cursor:
            ...
            needs_clause_embeddings = False
            clauses = doc.get("clauses", [])
            for clause in clauses:
                if not has_clause_embedding(clause):
                    needs_clause_embeddings = True
                    break
            if needs_doc_embedding or needs_clause_embeddings:
        ...
        def process_single_document(doc_data: Tuple[int, Dict[str, Any]]) -> Tuple[int, Dict[str, Any], bool, Optional[str]]:
            ...
            try:
                updated_doc, was_updated = process_document(
                    ...
                    clause_embedding_times=clause_embedding_times,
                ...
                try:
                    ...
                    if processed_count % 100 == 0 or processed_count == 1:
                        ...
                        with times_lock:
                            ...
                            avg_clause_time = sum(clause_embedding_times) / len(clause_embedding_times) if clause_embedding_times else 0
                            total_embeddings = len(doc_embedding_times) + len(clause_embedding_times)
