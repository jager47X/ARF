...
def parse_args():
    parser = argparse.ArgumentParser(description="Ingest Federal Register")
    parser.add_argument(
        "--production",
        action="store_true",
...
logger = logging.getLogger("ingest_federal_register")
BASE_DIR = Path(__file__).resolve().parents[2]
FEDERAL_REGISTER_DOCUMENT_PATH = str(BASE_DIR / "Data/Knowledge/federal_register.json")
FEDERAL_REGISTER_CONF = COLLECTION.get("FEDERAL_REGISTER_SET")
if not FEDERAL_REGISTER_CONF:
    raise ValueError("FEDERAL_REGISTER_SET not found in COLLECTION config. Please add FEDERAL_REGISTER_SET configuration to config.py")
DB_NAME: str = FEDERAL_REGISTER_CONF["db_name"]
COLL_NAME: str = FEDERAL_REGISTER_CONF["main_collection_name"]
...
def normalize_to_hierarchy(entry_obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    volume = entry_obj.get("volume", "")
    number = entry_obj.get("number", "")
    ...
    # Use clause number 1, clause title identical to document title for consistency
    return {
        "article": article,
        "section": section,
        "date": date,
...
def ingest():
    ...
    try:
        ...
        # Load Federal Register JSON
        data = load_json(FEDERAL_REGISTER_DOCUMENT_PATH)
        if not data:
            return
        items = data.get("data", {}).get("federal_register", {}).get("documents", [])
        if not items:
        ...
        # Deduplicate by title (document title)
        ...
        coll.create_index([("date", 1), ("volume", 1), ("number", 1)])
        coll.create_index("agency")
        # Unique across *clause titles* (multikey). Assumes each clause title is unique across Federal Register.
        coll.create_index("clauses.title", unique=True)
        logger.info("Indexes created: title (unique), (date,volume,number), agency, clauses.title (unique)")
